...
//handler for subscription (each changed on the server will be report on client)
static void
handler_currentTemp1(UA_Client *client, UA_UInt32 subId, void *subContext,
    ...
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "S1: %.2f", dato);
}
...
handler_currentTemp2(UA_Client *client, UA_UInt32 subId, void *subContext,
    ...
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "S2: %.2f", dato);
}
//handler for subscription (each changed on the server will be report on client)
static void
handler_currentTemp3(UA_Client *client, UA_UInt32 subId, void *subContext,
    ...
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "S3: %.2f", dato);
}
//handler for subscription (each changed on the server will be report on client)
static void
handler_currentTemp4(UA_Client *client, UA_UInt32 subId, void *subContext,
    ...
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "S4: %.2f", dato);
}
//delete subscription
static void
...
createsub(UA_Client *client) {
    ...
    UA_MonitoredItemCreateRequest mirTemp2 = UA_MonitoredItemCreateRequest_default(
        UA_NODEID_STRING(1, "Sensore2 di temperatura"));
    ...
    UA_MonitoredItemCreateRequest mirTemp4 = UA_MonitoredItemCreateRequest_default(
        UA_NODEID_STRING(1, "Sensore4 di temperatura"));
