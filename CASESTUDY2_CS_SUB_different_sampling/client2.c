...
handler_currentTemp1(UA_Client *client, UA_UInt32 subId, void *subContext,
    ...
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "P1: %.2f", dato);
}
...
createsub(UA_Client *client) {
    ...
    request.requestedPublishingInterval = 20000.0;  // 20 sec
    ...
    UA_MonitoredItemCreateRequest mirPress1 = UA_MonitoredItemCreateRequest_default(
        UA_NODEID_STRING(1, "Sensore di pressione Robot"));
    UA_MonitoredItemCreateResult monPress1 = UA_Client_MonitoredItems_createDataChange(
        client, response.subscriptionId, UA_TIMESTAMPSTORETURN_BOTH, mirPress1, NULL,
        handler_currentTemp1, NULL);

    if(monPress1.statusCode == UA_STATUSCODE_GOOD)
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                    "Monitoring UA_NS0ID_INTEGER, id %u", monPress1.monitoredItemId);
}
