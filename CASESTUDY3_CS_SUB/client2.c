...
handler_currentTemp1(UA_Client *client, UA_UInt32 subId, void *subContext,
    ...
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "S1 VELOCITY: %.2f", dato);
}
...
createsub(UA_Client *client) {
    ...
    UA_MonitoredItemCreateRequest mirTemp2 = UA_MonitoredItemCreateRequest_default(
        UA_NODEID_STRING(1, "Sensore2 di velocita"));
...
main(void) {
    ...
    while(running) {
        ...
        UA_StatusCode retval = UA_Client_connect(client, "opc.tcp://localhost:4841");
