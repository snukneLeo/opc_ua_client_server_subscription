...
createstructureVerticalWarehouse(UA_Server *server) {
    ...
    oAttr.displayName = UA_LOCALIZEDTEXT("en-US", "Server1 conveyor belt");
    UA_Server_addObjectNode(
        server, UA_NODEID_NULL, UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
        UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
        UA_QUALIFIEDNAME(1, "Conveyor Belt"),
    ...
    attrS2.description = UA_LOCALIZEDTEXT("en-US", "Pressure");
    attrS2.displayName = UA_LOCALIZEDTEXT("en-US", "Pressure");
    attrS2.dataType = UA_TYPES[UA_TYPES_FLOAT].typeId;
    attrS2.accessLevel = UA_ACCESSLEVELMASK_READ | UA_ACCESSLEVELMASK_WRITE;

    /* Add the variable node to the information model */
    UA_NodeId nodeS2 = UA_NODEID_STRING(1, "Sensore2 di pressione");
    UA_QualifiedName nodenameS2 = UA_QUALIFIEDNAME(1, "Sensore2 di pressione");
    ...
    attrS4.description = UA_LOCALIZEDTEXT("en-US", "vibration sensor");
    attrS4.displayName = UA_LOCALIZEDTEXT("en-US", "vibration sensor");
    attrS4.dataType = UA_TYPES[UA_TYPES_FLOAT].typeId;
    attrS4.accessLevel = UA_ACCESSLEVELMASK_READ | UA_ACCESSLEVELMASK_WRITE;

    /* Add the variable node to the information model */
    UA_NodeId nodeS4 = UA_NODEID_STRING(1, "Sensore4 di vibrazione");
    UA_QualifiedName nodenameS4 = UA_QUALIFIEDNAME(1, "Sensore4 di vibrazione");
    // UA_NodeId parentNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER);
...
//random function
double randomfun(double minvalue, double maxvalue)
{ 
    int valuerand = (rand() % ((int)maxvalue - (int)minvalue + 1)) + (int)minvalue; 
    return valuerand + 0.5; 
}
...
changevaluetemps1(void *vargp) {
    while(true) {
        writeVariable(server, randomfun(10.19,45.76), "Sensore1 di temperatura");
        if(valuetemp1 > 35)
            valuetemp1 = 22.5;
        sleep(10);  // 10 sec
    }
...
changevaluetemps2(void *vargp) {
    while(true) {
        writeVariable(server, randomfun(8.09,41.90), "Sensore2 di pressione");
        if(valuetemp2 > 40)
            valuetemp2 = 27.5;
        sleep(30);  // 30 sec
    }
...
changevaluetemps3(void *vargp) {
    while(true) {
        writeVariable(server, randomfun(20.90,47.01), "Sensore3 di temperatura");
        if(valuetemp3 > 45)
            valuetemp3 = 29.5;
        sleep(15);  // 15 sec
    }
...
changevaluetemps4(void *vargp) {
    while(true) {
        writeVariable(server, randomfun(17.09,40.99), "Sensore4 di vibrazione");
        if(valuetemp4 > 39)
            valuetemp4 = 17.5;
        sleep(60);  // 60 sec
    }
