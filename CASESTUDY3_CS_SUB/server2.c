...
createstructureVerticalWarehouse(UA_Server *server) {
    ...
    oAttr.displayName = UA_LOCALIZEDTEXT("en-US", "Server2 conveyor belt");
    ...
    attrS1.description = UA_LOCALIZEDTEXT("en-US", "Force Sensor");
    attrS1.displayName = UA_LOCALIZEDTEXT("en-US", "Force Sensor");
    ...
    UA_NodeId nodeS1 = UA_NODEID_STRING(1, "Sensore1 di forza");
    UA_QualifiedName nodenameS1 = UA_QUALIFIEDNAME(1, "Sensore1 di forza");
    ...
    attrS2.description = UA_LOCALIZEDTEXT("en-US", "Velocity sensor");
    attrS2.displayName = UA_LOCALIZEDTEXT("en-US", "Velocity sensor");
    attrS2.dataType = UA_TYPES[UA_TYPES_FLOAT].typeId;
    attrS2.accessLevel = UA_ACCESSLEVELMASK_READ | UA_ACCESSLEVELMASK_WRITE;

    /* Add the variable node to the information model */
    UA_NodeId nodeS2 = UA_NODEID_STRING(1, "Sensore2 di velocita");
    UA_QualifiedName nodenameS2 = UA_QUALIFIEDNAME(1, "Sensore2 di velocita");
...
changevaluetemps1(void *vargp) {
    while(true) {
        writeVariable(server, randomfun(20.6,37.09), "Sensore1 di forza");
        if(valuetemp1 > 35)
            valuetemp1 = 22.5;
        sleep(10);  // 10 sec
    }
...
changevaluetemps2(void *vargp) {
    while(true) {
        writeVariable(server, randomfun(19.9,45.19), "Sensore2 di velocita");
        if(valuetemp2 > 40)
            valuetemp2 = 27.5;
        sleep(30);  // 30 sec
    }
...
main(void) {
    ...
    UA_ServerConfig_setDefault_server2(UA_Server_getConfig(server));
